...
DEFAULT_REPORT = ROOT / ".codex_tmp" / "ficlit_technical_probe.csv"
DEFAULT_USER_AGENT = "ATK-Pro FICLIT technical probe (user-run local verification)"
...
def _classify_url(url: str) -> tuple[str, str, str] | None:
    ...
    if "dl.ficlit.unibo.it" in netloc:
        ...
        if "/s/lib/page/" in path_lower:
            return "portal_page", "ficlit_page", _identifier_from_path(path)
...
def _summarize(candidates: list[ProbeCandidate]) -> str:
    if not candidates:
        return "Nessun record FICLIT/Omeka, manifest, info.json, immagine IIIF o viewer candidato trovato."
    counts: dict[str, int] = {}
    roles: dict[str, int] = {}
    for candidate in candidates:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Sonda tecnica prudente per FICLIT Digital Library: cerca pagine "
            "Omeka, item/media/API, viewer Mirador, manifest IIIF, info.json "
            "e immagini candidate."
        )
    )
    parser.add_argument("--url", help="Pagina pubblica FICLIT/Omeka/IIIF da sondare.")
    ...
    try:
        ...
        print(f"ERRORE: impossibile leggere la pagina FICLIT: {exc}", file=sys.stderr)
        return 2
