def test_pdf_generator_completion9(monkeypatch, tmp_path):
    from src import pdf_generator as pg
    import os
    from PIL import Image

    # Setup immagini con ordine invertito rispetto al sort naturale
    img_dir = tmp_path / "imgs"
    img_dir.mkdir()
    img2 = img_dir / "img2.png"  # dovrebbe venire prima nel sort naturale
    ...
    class FakeImage:
        ...
        def verify(self):
            if "img1" in self.path:
                raise OSError("verify failed")  # riga 43
        def convert(self, mode):
            if "img2" in self.path:
                raise OSError("convert failed")  # riga 48
