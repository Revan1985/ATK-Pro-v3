def test_pdf_generator_completion8(monkeypatch, tmp_path):
    ...
    result1 = pg.run(args)
    assert result1["status"] == "ok" or result1["status"] == "error"

    # sort=mtime → riga 40
    args["output"] = img_dir / "out_mtime.pdf"
    args["sort"] = "mtime"
    result2 = pg.run(args)
    assert result2["status"] == "ok" or result2["status"] == "error"
