...
| `rovereto_digital_library` | Rovereto Digital Library / Biblioteca Tartarotti | Italia / Trentino | `https://digitallibrary.bibliotecacivica.rovereto.tn.it/`; campioni `https://digitallibrary.bibliotecacivica.rovereto.tn.it/entities/publication/e4199e9b-c79b-4c3d-b157-be2dcfc0407f`, `https://digitallibrary.bibliotecacivica.rovereto.tn.it/entities/picture/2cbeaeab-833a-48c2-9b39-29484ed1c681`; termini `https://digitallibrary.bibliotecacivica.rovereto.tn.it/info/end-user-agreement` | Media-alta per storia locale trentina, fotografie, documenti d'archivio, riviste e contesto familiare | Controllo 2026-06-07: portale operativo DSpace-GLAM trovato. Le pagine item pubbliche espongono Open in Mirador, IIIF/Open Manifest, file PDF/JPEG e licenza item-level; i campioni osservati sono CC BY-NC-ND. Il primo test live HTML non ha esposto link tecnici; la sonda ora classifica l'URL entity di input, deriva l'endpoint REST item standard, legge i link HAL/JSON delle sotto-risorse API (`bundles`, `thumbnail`, bundle e bitstream) e puo seguirli in modo limitato con `--follow-json`, senza aprire link di contenuto binario. Test live: 133 `bitstream_content` sul campione publication e 4 sul campione picture; aggiunto report qualitativo `--summarize-bitstreams` per metadati/formato/dimensione senza download contenuti, con classificazione di pagine `iiifpdf-*.png`, PDF originale, licenza, derivati testuali/OCR e miniature/cover. Termini ufficiali: accesso libero e gratuito ai file digitali in risoluzione web secondo licenze specifiche; alta definizione su richiesta | B - candidato tecnico promettente per PDF/JPEG puntuali e possibile IIIF `R_LIMITED`, ma prima servono verifica ordine/formato/duplicati dei bitstream e conferma licenza item-level per campione |
...
| `biblioteca_digitale_lombarda` | Italia / Lombardia | BDL/VuFind, record pubblici e PDF REST | Pista tecnica interessante: PDF REST pubblico verificato su campione, ma molti record sono CC BY-NC-ND; per ora solo PDF puntuale, immagini/IIIF sospesi per timeout su info.json |
...
| `rovereto_digital_library` | Italia / Trentino | portale DSpace-GLAM operativo, item pubblici, Mirador/IIIF, file PDF/JPEG e termini ufficiali | Candidato tecnico promettente; sonda dedicata aggiunta, valutare prima PDF/JPEG puntuali e poi eventuale `R_LIMITED` su manifest stabili |
