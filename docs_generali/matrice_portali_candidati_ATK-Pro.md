...
| `beic_digitale` | BEIC digital library | Italia / Lombardia | `https://digital.beic.it/`; `https://digital.beic.it/termini-duso/`; `https://digital.beic.it/infrastruttura-e-standard/`; esempi tecnici Preserver `https://preserver.beic.it/delivery/DeliveryManagerServlet?dps_pid=IE7400509` | Alta per libri antichi, manoscritti, periodici, editoria lombarda e fonti bibliografiche utili alla ricerca genealogica e territoriale | Controllo 2026-06-06: termini ufficiali molto favorevoli per opere in pubblico dominio, immagini scaricabili/riutilizzabili e dati CC0; infrastruttura Rosetta/Alma/Primo VE con OAI-PMH documentato. Lato download ancora da verificare su campioni Preserver/DeliveryManager: non e emerso per ora un manifest IIIF pubblico generalizzabile. Aggiunta sonda `verify_beic_technical_probe.py` per raccogliere record Primo, DeliveryManager, manifest, immagini e PDF senza integrare codice ATK-Pro | A/B - candidato forte per metadata e possibile download puntuale; prima eseguire sonda su piu campioni e distinguere DeliveryManager consultabile da endpoint file stabile |
| `biblioteca_digitale_lombarda` | Biblioteca Digitale Lombarda | Italia / Lombardia | `https://www.bdl.servizirl.it/`; esempi record pubblici `https://www.bdl.servizirl.it/vufind/Record/BDL-OGGETTO-133442`; endpoint PDF pubblici `https://www.bdl.servizirl.it/bdl/public/rest/srv/item/12404/pdf` | Alta per storia locale lombarda, periodici, monografie, archivi e fondi civici | Controllo 2026-06-06: portale pubblico VuFind con record e PDF REST pubblici; il campione `item/12404/pdf` risponde come PDF reale. La pagina record espone anche una miniatura Cantaloupe/IIIF (`6823693`) ma insieme ad asset grafici del sito: la sonda distingue ora immagini di contenuto, `info.json` derivato e asset. Molti record dichiarano CC BY-NC-ND, quindi uso studio/ricerca con limiti non commerciale/no-derivati | B - candidato tecnico interessante ma piu vincolato di BDT/BEIC; valutare prima PDF puntuale, poi eventuale `R_LIMITED` solo se info.json/immagini risultano stabili su piu campioni |
...
| `beic_digitale` | Italia / Lombardia | BEIC termini d'uso, infrastruttura/OAI-PMH, Primo VE e Preserver/DeliveryManager | Candidato forte per pubblico dominio, CC0 sui dati e metadata; sonda tecnica dedicata aggiunta, integrazione rinviata finche non sono verificati endpoint file/immagini o manifest su campioni reali |
| `biblioteca_digitale_lombarda` | Italia / Lombardia | BDL/VuFind, record pubblici e PDF REST | Pista tecnica interessante: PDF REST pubblico verificato su campione, ma con molti record CC BY-NC-ND; sonda dedicata aggiunta, valutare solo PDF puntuale/range limitato |
...
- Considerare avviato il sottoblocco biblioteche digitali italiane con
  `biblioteca_digitale_trentina`; proseguire con `beic_digitale`,
